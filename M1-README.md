...
Authenticated users can manage their subscriptions at `/markets`. The subscription page offers configured exchanges by their CCXT display names and loads spot pairs (XXX/YYY) when the exchange changes. It reads the price tick size from CCXT price precision; users do not type it. Available exchange candle periods appear for reference, while the shared feed selects its period automatically by information quality. Unsubscribe requests are scoped to the logged-in user. Users can reactivate an inactive subscription through the same add form. Some exchanges do not provide a fixed price increment for every pair; those pairs cannot be subscribed through the form until usable metadata is available.
